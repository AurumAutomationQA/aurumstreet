...
public class CustomerEnrollments extends ReusableMethods {
    ...
    @Test(priority = 1)
    public void navigateAndValidateCustomerEnrollments() {
        startTest("Navigate to Customer Enrollments and validate fields");

        click("//span[text()='Dashboard']/..", "dashboard button");
        clickByCSS(".max-xl\\3Ahidden > .w-full", "features expand");
        click("//span[text()='Customer Enrollments']", "Customer Enrollments link");

        verifyIsDisplayed("//h3[contains(text(),'Enrollments') or contains(text(),'Customer')]", "Customer Enrollments heading");
